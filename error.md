...
## 2026-08-28 — Workflow 화면의 밝은 레거시 카드 노출

### 증상

다크 디자인 셸에서 `수요 확정`, `재고 공급`, `마스터 검증`, `발주량 계산`, `보고 자료` 메뉴를 열면 내부 카드와 표가 흰색 레거시 디자인으로 표시되었습니다.

### 원인

해당 화면은 기존 Workflow 컴포넌트를 재사용하고 있었고, `card`, `table-wrap`, `report-preview` 등의 전역 레거시 CSS가 새 다크 디자인 토큰보다 우선 적용되었습니다.

### 해결책

계산 로직과 레거시 컴포넌트는 변경하지 않고, `.design-content .workflow-screen` 범위에만 다크 토큰을 적용했습니다. 카드, 표, 입력 영역, 호출 안내, 버튼과 보고서 미리보기의 배경·테두리·텍스트 색상이 공통 다크 토큰을 사용하도록 보정했습니다.

### 확인

`lib/workflow-dark-theme.test.ts` 회귀 테스트와 `npm run build`로 확인합니다.
