...
const columns: UiColumn<StockoutRisk>[] = [
  ...
  { key: 'plannedLeadTime', label: 'Effective Lead Time', align: 'right', render: (row) => numberCell(row.plannedLeadTime, '일', row.reason) },
  { key: 'stockoutPeriod', label: '소진예상 Period', align: 'right', render: (row) => row.stockoutPeriod ? row.stockoutPeriod : <EmptyValue reason={row.reason ?? 'NO_STOCKOUT'} /> },
  { key: 'daysOfSupply', label: 'Days of Supply', align: 'right', render: (row) => numberCell(row.daysOfSupply, '일', row.reason) },
  { key: 'monthsOfSupply', label: 'Months of Supply', align: 'right', render: (row) => numberCell(row.monthsOfSupply, '개월', row.reason) },
...
export default async function StockoutPage() {
  ...
  return <>
    <PageHeader eyebrow="ANALYSIS / STOCKOUT RISK" title="재고 소진 위험" description="Champion Forecast 기반 기간별 Inventory Projection으로 품목별 소진 위험을 확인합니다." action={<Badge status="critical">RISK MONITOR</Badge>} />
    <div className="kpi-grid">
      ...
      <KpiCard label="WARNING" value={kpi?.nWarning ?? <EmptyValue reason={combinedError ? 'QUERY_ERROR' : 'NO_DATA'} />} foot="Lead Time 내 대응 가능" status="warning" />
    </div>
